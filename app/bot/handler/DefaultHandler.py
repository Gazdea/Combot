...
async def anti_spam_protection(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if mute_user := muted_user_service.get_mute_user(message.chat.id, message.from_user.id):
...
async def remove_links(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
...
async def welcome_new_member(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    for member in message.new_chat_members:
        if member.id == context.bot.id:
            ...
            user_service.add_user(UserDTO(id=member.id, username=member.username))
